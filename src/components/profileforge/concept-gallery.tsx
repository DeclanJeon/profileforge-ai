...
function StylePresetCard({ preset, isSelected, onSelect }: { preset: StylePreset; isSelected: boolean; onSelect: () => void }) {
  return <Card className={cn('group cursor-pointer border-2 transition-all hover:shadow-md overflow-hidden', isSelected ? 'border-fuchsia-500 ring-2 ring-fuchsia-200 dark:ring-fuchsia-900' : 'border-border')} onClick={onSelect}>
    <div className="aspect-square bg-muted relative overflow-hidden">
      {preset.thumbnailPath ? <img src={preset.thumbnailPath} alt={`${preset.name} 적용 예시`} className="absolute inset-0 h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" onError={(e) => { const el = e.currentTarget; if (el.dataset.fallbackApplied) return; el.dataset.fallbackApplied = '1'; el.style.display = 'none'; const sib = el.nextElementSibling as HTMLElement | null; if (sib) sib.style.display = 'block' }} /> : null}<div className="absolute inset-0 bg-gradient-to-br from-fuchsia-500 via-rose-500 to-amber-400" style={{ display: preset.thumbnailPath ? 'none' : 'block' }} />
...
function ConceptCard({ concept, isSelected, onSelect }: { concept: Concept; isSelected: boolean; onSelect: () => void }) {
  ...
  return <Card className={cn('group overflow-hidden cursor-pointer transition-all border-2 hover:shadow-md', isSelected ? 'border-fuchsia-500 ring-2 ring-fuchsia-200 dark:ring-fuchsia-900' : 'border-border')} onClick={onSelect}>
    <div className="aspect-[4/5] bg-muted relative overflow-hidden"><img src={getConceptThumbnail(concept)} onError={(e) => { const el = e.currentTarget; if (el.dataset.fallbackApplied) return; el.dataset.fallbackApplied = "1"; el.src = "data:image/svg+xml," + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="500"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#d946ef"/><stop offset="1" stop-color="#f43f5e"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/></svg>') }} alt={`${concept.name} 예시 썸네일`} className="absolute inset-0 h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-black/15" /><div className="absolute top-2 left-2 right-2 flex items-start justify-between gap-1"><Badge variant="secondary" className="text-[10px] h-5">{CATEGORY_LABELS[concept.category]}</Badge><Badge variant={risk.variant} className="text-[10px] h-5">{risk.label}</Badge></div>{isSelected && <div className="absolute top-2 right-2"><div className="w-6 h-6 rounded-full bg-fuchsia-600 flex items-center justify-center"><CheckCircle2 className="w-4 h-4 text-white" /></div></div>}<div className="absolute bottom-0 inset-x-0 p-3 text-white"><h3 className="font-bold text-sm">{concept.name}</h3><p className="text-[10px] opacity-80 line-clamp-1 mt-0.5">{concept.styleTags.join(' · ')}</p></div></div>
